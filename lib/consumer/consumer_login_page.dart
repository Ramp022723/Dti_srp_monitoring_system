...
class ConsumerLoginPage extends StatefulWidget {
  const ConsumerLoginPage({Key? key}) : super(key: key);

  @override
  State<ConsumerLoginPage> createState() => _ConsumerLoginPageState();
}

class _ConsumerLoginPageState extends State<ConsumerLoginPage> with TickerProviderStateMixin {
  ...
  Future<void> _checkLoginStatus() async {
    ...
    if (isLoggedIn) {
      ...
      if (role != null && role.toLowerCase() == 'consumer' && mounted) {
  ...
  Future<void> _signIn() async {
    ...
    try {
      print('🛒 ConsumerLoginPage: Initiating consumer login');
      ...
      print('📋 ConsumerLoginPage: AuthService returned: $result');

      if (result['status'] == 'success') {
        final userName = result['user']['username'] ?? 'Consumer';
        final userRole = result['user']['role'] ?? 'consumer';
        
        print('✅ ConsumerLoginPage: Consumer login successful for $userName');
        
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Welcome back, $userName! Happy shopping!'),
        ...
        if (mounted) {
          print('🧭 ConsumerLoginPage: Navigating to consumer dashboard');
          DashboardService.navigateToDashboard(context, userRole);
        }
      } else {
        final errorMessage = result['message'] ?? 'Consumer login failed';
        final errorCode = result['code'] ?? 'UNKNOWN_ERROR';
        
        print('❌ ConsumerLoginPage: Login failed - $errorCode: $errorMessage');
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Welcome back! Sign in to browse products and make purchases from your favorite retailers.',
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  'Your shopping experience is secure and protected',
                  style: TextStyle(
                    fontSize: 12,
                    color: Colors.grey[600],
                  ),
